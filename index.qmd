...
subtitle: "Introducing and Estimating Regression"  
...
## Command Line
It can be started at the command line:
...
![radiant](./img/Screen Shot 2022-09-23 at 4.47.03 PM.png)
...
The generic z-transformation applied to a variable $x$ centers [mean$\approx$ 0] and scales [std. dev. $\approx$ variance $\approx$ 1] to $z_{x}$ for population parameters.  $\approx$ is approximately equal to.  In this case, two things are important.  First, this is the idea behind there only being one normal table in a statistics book.  The second is that, in this case, the $\mu$ and $\sigma$ are presumed known.
