...
```{r, fig.height=3.8, eval=FALSE}
library(fpp3)
recent_prices <- fpp3::prices %>% filter(!is.na(eggs)) %>% as_tsibble(index=year) %>% select(year, eggs)
recent_prices %>%
...
```{r combineplot, echo=TRUE, fig.height=4.2, eval=FALSE}
