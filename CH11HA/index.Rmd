...
```{r callsdata2, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
calls <- read_tsv("http://robjhyndman.com/data/callcenter.txt") %>%
  gather("date", "volume", -...1) %>% transmute(
    time = X1, date = as.Date(date, format = "%d/%m/%Y"),
    datetime = as_datetime(date) + time, volume) %>%
  as_tsibble(index = datetime)
calls %>%
...
```{r, fig.height=5, echo=FALSE, eval=FALSE}
library(sugrrants)
calls %>%
  filter(yearmonth(date) == yearmonth("2003 August")) %>%
  ggplot(aes(x = time, y = volume)) +
...
```{r sunspot-nnetar, echo=TRUE, fig.height=4, eval=FALSE}
...
```{r sunspot-nnetar-pi, echo=TRUE, fig.height=4, eval=FALSE}
fit %>% forecast(h=20) %>%
  autoplot(sunspots)
```
